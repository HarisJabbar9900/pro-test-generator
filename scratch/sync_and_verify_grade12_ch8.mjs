...
import { CHAPTER_8_NEW_TOPICS } from '../src/utils/chapter8TopicsData.js';
...
async function main() {
  ...
  const ch8 = cs.chapters.find(c => c.chapterNumber === 8);
  if (!ch8) {
    console.error("Grade 12 Chapter 8 not found!");
    process.exit(1);
  }

  // Ensure ch8 topics are updated with CHAPTER_8_NEW_TOPICS
  ch8.topics = JSON.parse(JSON.stringify(CHAPTER_8_NEW_TOPICS));
  ...
  console.log(`===========================================`);
  console.log(`Grade 12 - Chapter 8 Verification:`);
  ...
  if (totalShorts !== 40) {
    console.error(`ERROR: Expected 40 Shorts (30 topic + 10 exercise), got ${totalShorts}`);
    process.exit(1);
  }
  ...
  if (topicShorts !== 30) {
    console.error(`ERROR: Expected 30 Topic Shorts, got ${topicShorts}`);
    process.exit(1);
  }
  ...
  console.log("Firestore successfully written and verified with 40 MCQs (30 Topic + 10 Exercise) and 40 Shorts (30 Topic + 10 Exercise) for Grade 12 Chapter 8!");
  process.exit(0);
}
